...
router.get("/", async function (req, res) {
  const { statusCode, responseBody } = await salesService.getSales(
    req.query["token_id"]
  );
...
router.patch("/:tokenId/purchase", async function (req, res) {
  ...
  const { statusCode, responseBody } = await salesService.completeSales(
    tokenId,
    data
  );
...
router.delete("/:saleId", async function (req, res) {
  const { statusCode, responseBody } = await salesService.deleteSales(
    req.params["saleId"]
  );
