...
router.patch("/:itemId", async function (req, res) {
  ...
  const { statusCode, responseBody } =
    await itemService.updateItemTokenIdAndOwnerAddress(
      itemId,
      data["token_id"],
      data["owner_address"]
    );
...
router.get("/", async function (req, res) {
  const { statusCode, responseBody } = await itemService.getItems(
    req.query["address"]
  );
...
router.get("/:tokenId", async function (req, res) {
  const { statusCode, responseBody } = await itemService.getItemByTokenId(
    req.params["tokenId"]
  );
