...
- 테스트 계정
    ...
    3. 복구용 키를 입력한다
        - 복구용 키 : curve region monster purpose ride bulk egg caught crime energy law sunny
    4. 계정을 선택한다.
...
# 상태 확인
sudo system status redis-server
```
...
실제 사용할 포트와 인증서 경로에 맞춰 콜론(:) 기준 왼쪽 값을 변경하셔야합니다.
